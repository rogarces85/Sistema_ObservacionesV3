...
- Tablas principales: `usuarios`, `observaciones`, `historial_estados`, `establecimientos`, `comunas`.
- `config/config.php` contiene credenciales en texto plano — **no commitear en producción**.

## Rutas y permisos
- Páginas: `dashboard`, `observaciones`, `supervision`, `reportes`, `usuarios`, `perfil`.
- `supervision` y `usuarios` → solo rol `supervisor`. `registrador` es redirigido a `dashboard`.
