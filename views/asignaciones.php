...
<script>
    ...
    function renderizarListaRegistradores(registradores) {
        ...
        if (registradores.length === 0) {
            container.innerHTML = '<div class="empty"><div class="empty-icon"><i class="ti ti-users"></i></div><p class="empty-title">No hay registradores activos</p></div>';
            return;
        }
        ...
        Object.keys(porComuna).sort().forEach(comuna => {
            html += `<div class="mb-4">
                <div class="bg-slate-100 px-4 py-2 font-bold text-slate-700 text-sm rounded-t-lg">
                    <i class="ti ti-map-pin me-1"></i>${escapeHtml(comuna)}
                </div>`;
            ...
            porComuna[comuna].forEach(est => {
                ...
                if (est.tipo_asignacion === 'temporal' || (est.meses && est.meses !== 'ALL')) {
                    ...
                    tipoBadge = `<span class="text-xs font-semibold text-amber-700 bg-amber-100 px-2 py-0.5 rounded" title="Reasignación temporal: ${mesesTexto}"><i class="ti ti-clock me-1"></i>Temporal</span>`;
                } else {
        ...
        Object.keys(agrupados).sort().forEach(comuna => {
            ...
            agrupados[comuna].forEach(est => {
                ...
                if (asignadoAMi) {
                    ...
                    const tipoIcon = tipoMi === 'temporal' ? '<i class="ti ti-clock"></i>' : '<i class="ti ti-calendar"></i>';
                    ...
                    const tipoIcon = tipoOtro === 'temporal' ? '<i class="ti ti-clock"></i>' : '<i class="ti ti-calendar"></i>';
    ...
    async function cargarReasignacionesTemporales() {
        ...
        try {
            ...
            if (response.success && response.data.length > 0) {
                ...
                container.innerHTML = `
                    <div class="text-center py-8">
                        <div class="empty-icon"><i class="ti ti-clock-check"></i></div>
