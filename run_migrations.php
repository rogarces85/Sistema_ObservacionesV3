...
if (tableExists($pdo, 'asignaciones_establecimientos')) {
    if (!columnExists($pdo, 'asignaciones_establecimientos', 'anio')) {
        execSQL($pdo, 'Agregar columna anio', "ALTER TABLE asignaciones_establecimientos ADD COLUMN anio INT NOT NULL DEFAULT YEAR(CURDATE()) AFTER establecimiento_id");
    } else { echo "  ⏭️ Columna anio ya existe\n"; }
    if (!columnExists($pdo, 'asignaciones_establecimientos', 'meses')) {
    ...
    // Eliminar unique key antigua si existe (ya que ahora permitimos múltiples asignaciones por año/mes)
    if (indexExists($pdo, 'asignaciones_establecimientos', 'unique_asignacion')) {
        execSQL($pdo, 'Eliminar unique_asignacion obsoleta', "ALTER TABLE asignaciones_establecimientos DROP INDEX unique_asignacion");
    } else { echo "  ⏭️ unique_asignacion ya eliminada\n"; }
    if (!indexExists($pdo, 'asignaciones_establecimientos', 'idx_anio')) {
        execSQL($pdo, 'Agregar índice idx_anio', "ALTER TABLE asignaciones_establecimientos ADD INDEX idx_anio (anio)");
    } else { echo "  ⏭️ Índice idx_anio ya existe\n"; }
} else {
    echo "  ⚠️ Tabla asignaciones_establecimientos no existe (se creará más adelante)\n";
}
