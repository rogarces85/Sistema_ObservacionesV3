...
| Usuarios | Vista protegida para supervisores; listar; crear; editar; activar/desactivar; eliminar; restablecer contrasena; auditoria por usuario; endpoints en `api/users.php`. | Reset genera contrasena aleatoria de 12 caracteres con confirmacion explicita (`api/users.php:230`); la deuda restante son las cuentas semilla de `config/init_db.sql` que aun tengan `admin123`. | Implementado |
