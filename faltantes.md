...
| Observaciones | Listado; filtro por texto, estado y mes; crear; editar permitido; ver detalle; importacion Excel/CSV con preview y confirmacion; descarga de plantilla; validacion de asignacion mensual en registro manual. | La importacion no evidencia validacion mensual por fila; el modal no muestra historial de estados; no hay accion visible para eliminar aunque existe endpoint `DELETE`. | Parcial |
| Supervision | Vista protegida para supervisores; filtros; carga dinamica; detalle con historial; aprobar; cancelar/rechazar; mover a papelera; acciones masivas; clasificacion y detalle de error al aprobar. | La aprobacion masiva no exige seleccion explicita de resultado `S/OBSERVACION` o `ERROR` como si ocurre en aprobacion individual. | Parcial |
| Reportes | Filtros por anio, trimestre, mes, comuna y establecimiento; graficos/tablas de total errores, plazos, uso validador, serie y hoja; datos desde `api/reports.php`. | No hay botones de exportacion directa en la pagina; `api/export.php` existe pero no esta expuesto aqui; reportes asincronos no integrados. | Parcial |
| Usuarios | Vista protegida para supervisores; listar; crear; editar; activar/desactivar; eliminar; restablecer contrasena; endpoints en `api/users.php`. | No muestra auditoria/historial de cambios; reset usa contrasena fija `admin123`. | Parcial |
| Perfil | Visualizacion de datos del usuario; cambio de contrasena propia via `api/users.php`. | Actividad reciente es placeholder y no consulta historial real. | Parcial |
...
| Header / Navegacion | Selector de anio; cambio de tema light/dark; busqueda global basica de paginas; menu de usuario; logout. | Notificaciones es placeholder: badge `0` y texto estatico, sin backend real. | Parcial |
| Exportacion | Excel/PDF sincronico por `api/export.php`; informe errores JSON/PDF por `api/informe_errores.php`. | Cola asincrona de reportes no esta lista para produccion. | Parcial |
| Versionado tecnico | Existe `api/versioning.php` y `models/Version.php`. | No hay pagina visible en el router para administrar snapshots/rollback. | No expuesto |
